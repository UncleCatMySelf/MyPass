 ...
 * @QQ:1341933031
 * @Date:Created in 14:58 2018\8\27 0027
 */
@Slf4j
@Service
public class IUserPassServiceImpl implements IUserPassService {
    ...
    @Autowired
    public IUserPassServiceImpl(HbaseTemplate hbaseTemplate,MerchantsDao merchantsDao) {
    ...
    public Response userUsePass(Pass pass) {
        ...
        byte[] FAMILT_I = Constants.PassTable.FAMILY_I.getBytes();
        byte[] CON_DATE = Constants.PassTable.CON_DATE.getBytes();

        List<Mutation> datas = new ArrayList<>();

        Put put = new Put(passes.get(0).getRowKey().getBytes());
        put.addColumn(FAMILT_I,CON_DATE,
    ...
    private Map<String,PassTemplate> buildPassTemplateMap(List<Pass> passes) throws Exception{
        ...
        // 构造 PassTempalteId -》 PassTemplate Object 的 Map，用于构造 PassInfo
    ...
    private Response getPassInfoByStatus(Long userId, PassStatus status) throws Exception{
        ...
        if (status != PassStatus.ALL){
            filters.add(new SingleColumnValueFilter(
                    ...
                    Bytes.toBytes("-1")));// 等于 -1 未消费 or 不等于 -1 已消费
        }
